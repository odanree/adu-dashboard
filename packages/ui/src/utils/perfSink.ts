...
// receiving endpoint is POST /api/perf/profile — implemented on both
// backends (Python server.py + Hono packages/api/src/index.ts) so
// this works regardless of which one Caddy currently routes to.
//
// Pattern: production telemetry probe with fire-and-forget delivery.
// sendBeacon is the right primitive here — it's designed for the
// "commit-and-forget" shape, never blocks the render loop, and
// browsers guarantee delivery even during navigation.
//
// Sampling: only ships commits where actualDuration > SLOW_MS to keep
// the sink cheap. Everything faster is expected noise. Threshold is a
// knob; start conservative and tune based on what shows up in the
// first week of data.
...
function flush(): void {
  ...
  const url = `${import.meta.env.VITE_API_URL ?? ''}/api/perf/profile`
