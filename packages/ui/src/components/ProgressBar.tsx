...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  // Kick off the CSS width transition. requestAnimationFrame defers
  // setDisplayProgress until AFTER the initial 0-width paint, so the
  // browser can animate the transition. Same effect fires for any
  // subsequent `progress` change — matches the previous mount+update
  // behavior without needing a separate `mounted` state.
  useEffect(() => {
    const timer = requestAnimationFrame(() => setDisplayProgress(progress))
    return () => cancelAnimationFrame(timer)
  }, [progress])

  // JS-driven counter animation for the percentage text (CSS can't
  // animate the numeric value itself).
  useEffect(() => {
    ...
    const animate = () => {
      if (currentPercent < progress) {
        currentPercent += (progress - currentPercent) * 0.1
        if (currentPercent > progress - 0.5) {
